...
/**
 * Performance test for the UniqueOrderID entity.
 */
class UniqueOrderIDGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the UniqueOrderID entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all uniqueOrderIDS")
            .get("/order/api/unique-order-ids")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new uniqueOrderID")
            .post("/order/api/unique-order-ids")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_uniqueOrderID_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created uniqueOrderID")
                .get("/order${new_uniqueOrderID_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created uniqueOrderID")
            .delete("/order${new_uniqueOrderID_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
