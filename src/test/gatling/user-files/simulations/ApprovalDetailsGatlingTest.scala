...
/**
 * Performance test for the ApprovalDetails entity.
 */
class ApprovalDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ApprovalDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all approvalDetails")
            .get("/order/api/approval-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new approvalDetails")
            .post("/order/api/approval-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "acceptedAt":"2020-01-01T00:00:00.000Z"
                , "expectedDelivery":"SAMPLE_TEXT"
                , "decision":"SAMPLE_TEXT"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_approvalDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created approvalDetails")
                .get("/order${new_approvalDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created approvalDetails")
            .delete("/order${new_approvalDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
